...
function main_task()
    ...
    function eposes = calculate_xyz(joint_space_waypoints)
    
        num_waypoints = size(joint_space_waypoints, 1);
        eposes = zeros(num_waypoints, 3);

        for i = 1:num_waypoints
            eePose = getTransform(Px100.robot, [joint_space_waypoints(i,:), 0, 0, 0], 'px100/ee_gripper_link');
            eposes(i, :) = eePose(1:3, 4)';
        end
